...
- (void)setUp {
    ...
    [[MAIService sharedInstance] saveItineraries:_itineraries withSuccessDataHandler:nil withFailureDataHandler:nil];
}
...
- (void)testSearchAddresses {
    ...
    [[MAIService sharedInstance] search:@"200 Randolph Street, Chicago" withDataSourceCompletionHandler:^(NSArray* items){
...
- (void)testSearchPlaces {
    ...
    [[MAIService sharedInstance] search:@"Eiffel Tower" withDataSourceCompletionHandler:^(NSArray* items){
        XCTAssert(items.count>0);
        [expectation fulfill];
    } withDataSourceErrorHandler:^(NSString* errorMessage){
        XCTFail(@"%@", errorMessage);
...
- (void) testSaveNewItinerary {
    ...
    [[MAIService sharedInstance] saveItinerary:anItinerary withSuccessDataHandler:^(MAIItinerary *amendedItinerary){
...
- (void) testSaveExistingItinerary {
...
- (void) testSaveItineraries {
    ...
    [[MAIService sharedInstance] saveItineraries:_itineraries withSuccessDataHandler:^void{
        [expectation fulfill];
    } withFailureDataHandler:^(NSString *errorMessage) {
        XCTFail(@"%@", errorMessage);
        [expectation fulfill];
...
- (void) testCalculateRoute {
    ...
    [[MAIService sharedInstance] calculateRoute:routeItinerary withSuccessDataHandler:^(MAIRoute *route) {
...
- (void) testGetRoute {
    ...
    [[MAIService sharedInstance] getRoute:route withSuccessDataHandler:^(MAIRoutePolyline *routePolyline) {
